...
export async function GET(request: Request) {
  ...
  try {
    ...
    const campaignResponse = await metaGraphFetch(`/${accountId}/campaigns`, accessToken.value, { params: campaignParams })
    if (!campaignResponse.ok) {
      const errorData = await campaignResponse.json().catch(() => ({}))
      return NextResponse.json({ error: 'meta_api_error', details: errorData.error || { message: `HTTP ${campaignResponse.status}` } }, { status: 502 })
    }

    const campaignData = await campaignResponse.json()
    const campaigns = campaignData.data || []
    if (campaigns.length === 0) return NextResponse.json({ data: [], paging: {} })
