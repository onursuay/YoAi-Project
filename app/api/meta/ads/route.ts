...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!adResponse.ok) {
      ...
      return NextResponse.json({ error: 'meta_api_error', details: errorData.error || { message: `HTTP ${adResponse.status}` } }, { status: 502 })
    }

    const adData = await adResponse.json()
    const ads = adData.data || []
    if (ads.length === 0) return NextResponse.json({ data: [], paging: {} })
    ...
    if (insightsResponse.ok) {
      ...
      insights.forEach((insight: any) => {
        if (insight.ad_id) insightsMap.set(insight.ad_id, insight)
      })
    }
